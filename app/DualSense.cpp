	...
	if (devInfo != INVALID_HANDLE_VALUE)
	{
		...
		while (SetupDiEnumDeviceInfo(devInfo, devIndex, &devInfoData))
		{
			...
			SetupDiGetDeviceInstanceIdW(devInfo, &devInfoData,
										(PWSTR)deviceInstanceId.data(), MAX_PATH, 0);
...
DualsenseUtils::InputFeatures DualsenseUtils::InputFeatures::from_json(
	const nlohmann::json &j)
{
	InputFeatures features;
#define X(name)            \
	if (j.contains(#name)) \
		j.at(#name).get_to(features.name);
	INPUTFEATURES_JSON_MEMBERS
#undef X
	return features;
}

bool DualsenseUtils::InputFeatures::operator==(
	const InputFeatures &other) const
{
	return VibrationType == other.VibrationType && Features == other.Features &&
		   _RightMotor == other._RightMotor && _LeftMotor == other._LeftMotor &&
		   SpeakerVolume == other.SpeakerVolume &&
		   MicrophoneVolume == other.MicrophoneVolume &&
		   HeadsetVolume == other.HeadsetVolume &&
		   audioOutput == other.audioOutput && micLED == other.micLED &&
		   micStatus == other.micStatus &&
		   RightTriggerMode == other.RightTriggerMode &&
		   LeftTriggerMode == other.LeftTriggerMode &&
		   std::equal(std::begin(RightTriggerForces),
					  std::end(RightTriggerForces),
					  std::begin(other.RightTriggerForces)) &&
		   std::equal(std::begin(LeftTriggerForces), std::end(LeftTriggerForces),
					  std::begin(other.LeftTriggerForces)) &&
		   _Brightness == other._Brightness && playerLED == other.playerLED &&
		   PlayerLED_Bitmask == other.PlayerLED_Bitmask && Red == other.Red &&
		   Green == other.Green && Blue == other.Blue &&
		   ImprovedRumbleEmulation == other.ImprovedRumbleEmulation &&
		   RumbleReduction == other.RumbleReduction && ID == other.ID;
}
...
std::string convertDeviceId(const std::string &originalId)
{
	...
	std::string instancePart =
		trimmedId.substr(hashPos + 1, nextHashPos - hashPos - 1);
...
void debugString(const std::wstring &str)
{
	...
	{
		std::wcout << L"[" << i << L"]: '" << str[i] << L"' (0x" << std::hex
				   << (int)str[i] << std::dec << L")" << std::endl;
	}
}
...
bool compareDeviceIDs(const std::wstring &id1, const std::wstring &id2)
{
	...
	wcout << L"Comparing -> " << strippedId1 << L" size " << strippedId1.size()
		  << L" to " << strippedId2 << L" size " << strippedId2.size() << endl;
	return strippedId1 == strippedId2;
}

void Dualsense::data_callback(ma_device *device, void *output,
							  const void *input, ma_uint32 frameCount)
{
	...
	for (ma_uint32 i = 0; i < frameCount; ++i)
	{
		for (int channel = 0; channel < 4; ++channel)
		{
			...
			{
				if (channel == 0 && sample > self->peaks.channelZero)
					self->peaks.channelZero = sample;
				else if (channel == 1 && sample > self->peaks.Speaker)
					self->peaks.Speaker = sample;
				else if (channel == 2 && sample > self->peaks.LeftActuator)
					self->peaks.LeftActuator = sample;
				else if (channel == 3 && sample > self->peaks.RightActuator)
					self->peaks.RightActuator = sample;
			}
		}
	}
}

void Dualsense::data_callback_playback(ma_device *device, void *output,
									   const void *input,
									   ma_uint32 frameCount)
{
	...
	if (self->TouchpadToHaptics && self->State.trackPadTouch0.IsActive)
	{
		...
		const float phaseIncrement =
			(2.0f * 3.14f * self->TouchpadToHapticsFrequency) / 48000.0f;
...
Dualsense::Dualsense(std::string Path)
	...
	if (Path != "")
	{
		...
		else
		{
			...
			if (connectionType == Feature::USB)
			{
				...
				int statusf = FindParentDeviceInstanceId(replaceUSBwithHID(finalS),
														 L".*", parent);
...
bool Dualsense::LoadSound(const std::string &soundName,
						  const std::string &filePath)
{
	if (connectionType == Feature::USB && AudioInitialized &&
		!AudioDeviceNotFound)
	{
		if (soundMap.find(soundName) != soundMap.end())
			return false;
		shared_ptr<ma_sound> sound = make_shared<ma_sound>();
		ma_result result = ma_sound_init_from_file(&engine, filePath.c_str(), 0,
												   NULL, NULL, sound.get());
		...
		{
			std::cerr << "Failed to load " << filePath << ". Reason: " << result
					  << std::endl;
	...
	if (handle && Connected)
	{
		...
		else
		{
			...
			buttonState.trackPadTouch0.X =
				((ButtonStates[35 + offset] & 0x0F) << 8) | ButtonStates[34 + offset];
			buttonState.trackPadTouch0.Y = ((ButtonStates[36 + offset]) << 4) |
			...
			buttonState.trackPadTouch1.X =
				((ButtonStates[39 + offset] & 0x0F) << 8) | ButtonStates[38 + offset];
			buttonState.trackPadTouch1.Y = ((ButtonStates[40 + offset]) << 4) |
										   ((ButtonStates[39 + offset] & 0xF0) >> 4);
			buttonState.TouchPacketNum = (uint8_t)(ButtonStates[41 + offset]);
			buttonState.accelerometer.X = bit_cast<int16_t>(array<uint8_t, 2>{
				ButtonStates[16 + offset], ButtonStates[17 + offset]});
			buttonState.accelerometer.Y = bit_cast<int16_t>(array<uint8_t, 2>{
				ButtonStates[18 + offset], ButtonStates[19 + offset]});
			buttonState.accelerometer.Z = bit_cast<int16_t>(array<uint8_t, 2>{
				ButtonStates[20 + offset], ButtonStates[21 + offset]});
			buttonState.gyro.X = bit_cast<int16_t>(array<uint8_t, 2>{
				ButtonStates[22 + offset], ButtonStates[23 + offset]});
			buttonState.gyro.Y = bit_cast<int16_t>(array<uint8_t, 2>{
				ButtonStates[24 + offset], ButtonStates[25 + offset]});
			buttonState.gyro.Z = bit_cast<int16_t>(array<uint8_t, 2>{
				ButtonStates[26 + offset], ButtonStates[27 + offset]});
...
bool Dualsense::Write()
{
	...
	if (handle != nullptr && Connected)
	{
		...
		if (connectionType == Feature::USB)
		{
			...
			for (int i = 0; i < 10; i++)
				outReport[12 + i] = CurSettings.RightTriggerForces[i];
			outReport[22] = CurSettings.LeftTriggerMode;
			for (int i = 0; i < 10; i++)
				outReport[23 + i] = CurSettings.LeftTriggerForces[i];
			outReport[37] = (0x0 << 4) | (CurSettings.RumbleReduction & 0x0F);
			outReport[39] ^= (1 << 0); // AllowLightBrightnessChange = 1
			outReport[39] ^=
				CurSettings.ImprovedRumbleEmulation ? (1 << 2) : (0 << 2);
		...
		else if (connectionType == Feature::BT)
		{
			...
			for (int i = 0; i < 10; i++)
				outReport[13 + i] = CurSettings.RightTriggerForces[i];
			outReport[23] = CurSettings.LeftTriggerMode;
			for (int i = 0; i < 10; i++)
				outReport[24 + i] = CurSettings.LeftTriggerForces[i];
...
void Dualsense::InitializeAudioEngine()
{
	...
	if ((Now - LastTimeReconnected) < 2s)
		return;
	if (!AudioInitialized && !AudioDeviceNotFound &&
		connectionType == Feature::USB && Connected)
	{
		...
		if (FAILED(hr))
			std::wcerr << L"Failed to create IMMDeviceEnumerator." << std::endl;
		hr = pEnumerator->EnumAudioEndpoints(eRender, DEVICE_STATE_ACTIVE,
											 &pDevices);
		if (FAILED(hr))
			std::wcerr << L"Failed to enumerate audio endpoints: " << std::hex << hr
					   << std::endl;
		UINT count;
		...
		for (UINT i = 0; i < count; ++i)
		{
			...
			{
				std::wcerr << L"Failed to get device at index " << i << L": "
						   << std::hex << hr << std::endl;
				continue;
			}
			hr = pDevice->GetId(&ID);
			...
			{
				wcout << L"Found matching parents for dualsense speaker: " << foundID
					  << endl;
				found = true;
		...
		for (ma_uint32 iDevice = 0; iDevice < playbackCount; iDevice += 1)
		{
			...
			ma_result r =
				ma_context_get_device_info(&context, ma_device_type_playback,
										   &pPlaybackInfos[iDevice].id, &info);
			if (wcscmp(info.id.wasapi, ID) == 0)
			{
				wcout << L"Selected audio device: " << info.id.wasapi << L" Index:"
					  << deviceIndex << endl;
				found = true;
				break;
			}
		...
		ma_device_config deviceconfigLoopback =
		...
		if (ma_device_init(NULL, &deviceconfigLoopback, &deviceLoopback) !=
			MA_SUCCESS)
		{
...
{
	for (auto it = CurrentlyPlayedSounds.begin();
		 it != CurrentlyPlayedSounds.end();)
	{
...
bool Dualsense::PlayHaptics(const std::string &soundName,
							bool DontPlayIfAlreadyPlaying,
							bool LoopUntilStoppedManually)
{
	if (connectionType == Feature::USB && AudioInitialized &&
		!AudioDeviceNotFound)
	{
		...
		{
			if (ma_sound_is_playing(it2->second.get()) ||
				ma_sound_is_looping(it2->second.get()))
			{
		...
		ma_result result = ma_sound_init_copy(&engine, originalSound.get(), 0, NULL,
											  newSound.get());
		if (result != MA_SUCCESS)
		{
			soundMap.erase(it);
			std::cerr << "Failed to create a new sound instance. Error: " << result
					  << std::endl;
			return false;
		}
		...
		{
			std::cerr << "Error playing sound: " << static_cast<int>(soundplay)
					  << std::endl;
...
void Dualsense::SetMicrophoneVolume(int Volume)
{
...
void Dualsense::SetPlayerLED(uint8_t Player)
{
	...
	case 2:
		...
		break;
	case 3:
		CurSettings.PlayerLED_Bitmask = 0x08;
		break;
	case 4:
		CurSettings.PlayerLED_Bitmask = 0x01;
		break;
	case 5:
		CurSettings.PlayerLED_Bitmask = 0x16;
		break;
	default:
		CurSettings.PlayerLED_Bitmask = 0x00;
		break;
...
void Dualsense::SetRightTrigger(Trigger::TriggerMode triggerMode,
								...
								uint8_t Force7, uint8_t Force8, uint8_t Force9,
								uint8_t Force10, uint8_t Force11)
{
...
void Dualsense::SetLeftTrigger(Trigger::TriggerMode triggerMode, uint8_t Force1,
...
{
	CurSettings.VibrationType =
		flag ? Feature::StandardRumble : Feature::Haptic_Feedback;
}

void Dualsense::SetSpeakerVolume(int Volume)
{
	CurSettings.SpeakerVolume = Volume;
}
void Dualsense::SetHeadsetVolume(int Volume)
{
...
{
	CurSettings.audioOutput =
		flag ? Feature::AudioOutput::Headset : Feature::AudioOutput::Speaker;
}
...
void Dualsense::Remove()
{
	isRemoved = true;
}

bool Dualsense::IsRemoved() const
{
	return isRemoved;
}

void Dualsense::SetDisableLightbar(bool value)
{
	disableLightbar = value;
	if (disableLightbar)
	{
		CurSettings.Features = Feature::FeatureType::NoRGB;
	}
	else
	{
		CurSettings.Features = Feature::FeatureType::Full;
	}
}