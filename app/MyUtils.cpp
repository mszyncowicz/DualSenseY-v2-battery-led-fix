...
namespace MyUtils
{
    void StartAudioToHaptics(const std::string &Name)
    {
        ...
        std::wcout << L"Starting audio to haptics" << std::endl
                   << L"Arg1: " << arg1 << std::endl;
    ...
    void AddToStartup()
    {
        ...
        if (createStatus == ERROR_SUCCESS)
        {
            ...
            long status = RegSetValueEx(hkey, "MakeSense", 0, REG_SZ, (BYTE *)progPath.c_str(), (progPath.size() + 1) * sizeof(wchar_t));
    ...
    void RemoveFromStartup()
    {
        ...
        if (openStatus == ERROR_SUCCESS)
        {
            ...
            long deleteStatus = RegDeleteValue(hkey, "MakeSense");
            RegCloseKey(hkey); // Close the key

            if (deleteStatus == ERROR_SUCCESS)
            {
                std::cout << "Successfully removed from startup: " << "MakeSense" << std::endl;
            }
            else
            {
    ...
    {
        HKEY hkey;
        const char *subkey = "Software\\Classes\\AppUserModelId\\MakeSense";
        ...
        // Remove the executable filename to get the folder path
        if (!PathRemoveFileSpecA(exePath))
        {
            std::cerr << "Failed to strip executable filename." << std::endl;
            return;
        }

        // Construct icon path: <exe_folder>\utilities\icon.ico
        std::string iconPath(exePath);
        iconPath += "\\utilities\\icon.ico";

        // Create or open the registry key
        long createStatus = RegCreateKeyExA(HKEY_CURRENT_USER, subkey, 0, NULL, 0, KEY_SET_VALUE, NULL, &hkey, NULL);
        if (createStatus == ERROR_SUCCESS)
        {
            const char *displayName = "MakeSense";
            const char *iconBgColor = "AARRGGBB";

            RegSetValueExA(hkey, "DisplayName", 0, REG_SZ, (const BYTE *)displayName, (DWORD)(strlen(displayName) + 1));
            RegSetValueExA(hkey, "IconUri", 0, REG_SZ, (const BYTE *)iconPath.c_str(), (DWORD)(iconPath.length() + 1));
            RegSetValueExA(hkey, "IconBackgroundColor", 0, REG_SZ, (const BYTE *)iconBgColor, (DWORD)(strlen(iconBgColor) + 1));

            RegCloseKey(hkey);
            std::cout << "Successfully added AppUserModelId registry values." << std::endl;
        }
        else
        {
    ...
    void RemoveNotifications()
    {
        ...
        // Delete the entire key (and all values under it)
        long deleteStatus = RegDeleteKeyA(HKEY_CURRENT_USER, subkey);
        if (deleteStatus == ERROR_SUCCESS)
        {
            std::cout << "Successfully removed AppUserModelId registry key." << std::endl;
        }
        else
        {
            std::cerr << "Failed to delete AppUserModelId registry key. It may not exist or require elevated permissions." << std::endl;
        }
    }
