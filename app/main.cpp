...
void setTriggerSettings(TriggerSettings &trigger,
                        const char *modeStr,
                        const int *feedbackArray,
                        size_t feedbackSize)
{
    ...
    else if (strcmp(modeStr, "Vibration") == 0)
    {
        ...
        if (amplitude > 0 && frequency > 0)
        {
            ...
            trigger.Forces[2] =
                static_cast<uint8_t>((amplitudeZones >> 0) & 0xFFF);
            trigger.Forces[3] =
                static_cast<uint8_t>((amplitudeZones >> 8) & 0xFF);
            trigger.Forces[4] =
                static_cast<uint8_t>((amplitudeZones >> 16) & 0xFF);
            trigger.Forces[5] =
                static_cast<uint8_t>((amplitudeZones >> 24) & 0xFF);
            trigger.Forces[6] = 0;
            trigger.Forces[7] = 0;
    ...
    else if (strcmp(modeStr, "Slope Feedback") == 0)
    {
        ...
        for (int i = startPosition; i < 10; i++)
        {
            ...
            {
                float strengthFloat =
                    static_cast<float>(startStrength) +
...
void writeEmuController(Dualsense &controller, Settings &settings)
{
    ...
    constexpr auto DISCONNECT_DELAY =
        std::chrono::seconds(5); // 5 seconds delay
    ...
    while (!stop_thread)
    {
        if (controller.Connected)
        {
            ...
            else if (wasVirtualControllerStarted &&
                     settings.emuStatus != lastEmu)
            {
                v.Remove360();
                v.RemoveDS4();
                lastEmu == None;
            ...
            if (settings.TouchpadToRXRY &&
                controller.State.trackPadTouch0.IsActive)
            {
                editedButtonState.RX =
                    ConvertRange(controller.State.trackPadTouch0.X,
                                 0,
                                 1919,
                                 0,
                                 255);
                editedButtonState.RY =
                    ConvertRange(controller.State.trackPadTouch0.Y,
                                 0,
                                 1079,
                                 0,
                                 255);
                    ...
                    fabs(controller.State.RY - 128) <= 80)
                {
                    ...
                    else
                    {
                        normalizedAccelX =
                            (normalizedAccelX > 0
                                 ...
                                 : normalizedAccelX +
                                       settings
                                           .GyroToRightAnalogStickDeadzone) /
                    ...
                    if (adjustedX > 0)
                    {
                        accelAnalogStickX =
                            ...
                            max(static_cast<int>(
                                    settings
                                        .GyroToRightAnalogStickMinimumStickValue),
            ...
            {
                int AnalogX =
                    settings.LeftStickToMouse ? controller.State.LX : 0;
                AnalogX =
                    settings.RightStickToMouse ? controller.State.RX : AnalogX;

                int AnalogY =
                    settings.LeftStickToMouse ? controller.State.LY : 0;
                AnalogY =
                    settings.RightStickToMouse ? controller.State.RY : AnalogY;

                float x =
                    (AnalogX - 128) * settings.LeftStickToMouseSensitivity;
                float y = (AnalogY - 128) *
                          (settings.LeftStickToMouseSensitivity + 0.001f);
...
void writeControllerState(Dualsense &controller,
                          Settings &settings,
                          UDP &udpServer)
{
    SetThreadPriority(GetCurrentThread(), THREAD_PRIORITY_HIGHEST);
    ...
    while (!stop_thread)
    {
        if (true)
        {
                 ...
                 settings.DisconnectControllerShortcut))
            {
                ...
                if ((now - lastTimeFailedToWrite) > std::chrono::seconds(5))
                {
                    MyUtils::DisableBluetoothDevice(
                        controller.GetMACAddress(false));
            ...
            if (controller.State.micBtn && controller.State.L2 &&
                settings.GyroToRightAnalogStickShortcut &&
                !MicShortcutTriggered)
            {
            ...
            {
                settings.TouchpadToMouse =
                    settings.TouchpadToMouse ? settings.TouchpadToMouse = false
                ...
                controller.State.trackPadTouch0.IsActive)
            {
                ...
                if (controller.State.trackPadTouch1.IsActive)
                {
                    ...
                    {
                        MouseClick(MOUSEEVENTF_WHEEL,
                                   static_cast<int>(delta1Y * 2));
                    }
                ...
                !settings.CurrentlyUsingUDP && settings.triggerFormat == "DSX")
            {
                ...
                controller.SetLeftTrigger(
                    settings.ControllerInput.LeftTriggerMode,
...
#pragma region Apply Sony Trigger
                ...
                {
                    setTriggerSettings(l2,
                                       settings.lmodestrSony.c_str(),
                                       settings.L2Feedback,
                                       sizeof(settings.L2Feedback));
                }
                ...
                {
                    setTriggerSettings(r2,
                                       settings.rmodestrSony.c_str(),
                                       settings.R2Feedback,
                                       sizeof(settings.R2Feedback));
                }
            ...
            skipSonyTrigger:
                controller.SetRightTrigger(r2.Mode,
                                           r2.Forces[0],
                                           r2.Forces[1],
                                           r2.Forces[2],
                                           r2.Forces[3],
                ...
                controller.SetLeftTrigger(l2.Mode,
                                          l2.Forces[0],
                                          l2.Forces[1],
                                          l2.Forces[2],
                                          l2.Forces[3],
...
#pragma endregion
            ...
            if (settings.RumbleToAT && !settings.CurrentlyUsingUDP)
            {
                ...
                if (settings.RumbleToAT_RigidMode)
                {
                    ...
                    leftForces[1] = settings.SwapTriggersRumbleToAT
                                        ? min(settings.MaxRightTriggerRigid,
                                              settings.rrumble)
                                        : min(settings.MaxLeftTriggerRigid,
                                              settings.lrumble);
                    ...
                    controller.SetLeftTrigger(Trigger::Rigid,
                                              leftForces[0],
                                              leftForces[1],
                                              0,
                                              0,
                                              ...
                                              0,
                                              0);
                    controller.SetRightTrigger(Trigger::Rigid,
                                               rightForces[0],
                                               rightForces[1],
                                               0,
                                               0,
                ...
                else
                {
                    ...
                    if (!settings.SwapTriggersRumbleToAT &&
                        ...
                        leftForces[0] =
                            min(settings.MaxLeftRumbleToATFrequency, 25);
                    ...
                    if (!settings.SwapTriggersRumbleToAT &&
                        ...
                        rightForces[0] =
                            min(settings.MaxRightRumbleToATFrequency, 25);

                    // set power
                    leftForces[1] =
                        settings.SwapTriggersRumbleToAT
                            ? min(settings.MaxRightRumbleToATIntensity,
                                  settings.rrumble)
                            : min(settings.MaxLeftRumbleToATIntensity,
                                  settings.lrumble);
                    rightForces[1] =
                        settings.SwapTriggersRumbleToAT
                            ? min(settings.MaxLeftRumbleToATIntensity,
                    ...
                    controller.SetLeftTrigger(Trigger::Pulse_B,
                                              leftForces[0],
                                              leftForces[1],
                                              leftForces[2],
                                              0,
                                              ...
                                              0,
                                              0);
                    controller.SetRightTrigger(Trigger::Pulse_B,
                                               rightForces[0],
                                               rightForces[1],
                                               rightForces[2],
                                               0,
            ...
            if (settings.HapticsToTriggers && !settings.CurrentlyUsingUDP)
            {
                ...
                int ScaledLeftActuator = MyUtils::scaleFloatToInt(
                    peaks.LeftActuator,
                    settings.AudioToTriggersMaxFloatValue);
                ...
                else
                {
                    ...
                    // set frequency
                    if (!settings.SwapTriggersRumbleToAT &&
                            ScaledLeftActuator < 25 ||
                        settings.SwapTriggersRumbleToAT &&
                            ScaledRightActuator < 25)
                        leftForces[0] =
                            settings.SwapTriggersRumbleToAT
                                ? min(settings.MaxRightRumbleToATFrequency,
                                      ScaledRightActuator)
        ...
        skiphapticstotriggers:
            ...
            if (settings.FullyRetractTriggers)
            {
                if (settings.RumbleToAT || settings.RumbleToAT_RigidMode)
                {
                    ...
                    {
                        if (!settings.SwapTriggersRumbleToAT)
                            controller.SetLeftTrigger(Trigger::Rigid_B,
                                                      0,
                                                      0,
                                                      0,
                                                      0,
                            ...
                            controller.SetRightTrigger(Trigger::Rigid_B,
                                                       0,
                                                       0,
                                                       0,
                                                       0,
        ...
        skipretract:
                ...
                X360animationplayed)
            {
                ...
                {
                    float t =
                        static_cast<float>(peak) / settings.QUIET_THRESHOLD;
                ...
                else if (peak <= settings.MEDIUM_THRESHOLD)
                {
                    ...
                    r = static_cast<int>(settings.QUIET_COLOR[0] +
                                         t * (settings.MEDIUM_COLOR[0] -
                                              settings.QUIET_COLOR[0]));
                    g = static_cast<int>(settings.QUIET_COLOR[1] +
                                         t * (settings.MEDIUM_COLOR[1] -
                                              settings.QUIET_COLOR[1]));
                    b = static_cast<int>(settings.QUIET_COLOR[2] +
                                         t * (settings.MEDIUM_COLOR[2] -
                                              settings.QUIET_COLOR[2]));
                }
                else
                {
                    float t =
                        static_cast<float>(peak - settings.MEDIUM_THRESHOLD) /
                        (255 - settings.MEDIUM_THRESHOLD);
                    r = static_cast<int>(settings.MEDIUM_COLOR[0] +
                                         t * (settings.LOUD_COLOR[0] -
                                              settings.MEDIUM_COLOR[0]));
                    g = static_cast<int>(settings.MEDIUM_COLOR[1] +
                                         t * (settings.LOUD_COLOR[1] -
                                              settings.MEDIUM_COLOR[1]));
                    b = static_cast<int>(settings.MEDIUM_COLOR[2] +
                                         t * (settings.LOUD_COLOR[2] -
                ...
                X360animationplayed)
            {
                ...
                {
                    batteryLevel =
                        std::max<int>(0, std::min<int>(100, batteryLevel));
            ...
            if (settings.MicScreenshot)
            {
                    ...
                    !controller.State.L1 && !controller.State.L2)
                {
                    ...
                    filesystem::create_directories(
                        MyUtils::GetImagesFolderPath() + "\\DualSenseY\\");
                    string filename = MyUtils::GetImagesFolderPath() +
                                      "\\DualSenseY\\" +
            ...
            controller.EnableImprovedRumbleEmulation(settings.ControllerInput.ImprovedRumbleEmulation);
...
{
    tray.addEntry(
        Tray::Button("Show window", [&] { AppConfig.ShowWindow = true; }));
...
int main()
{
        ...
        try
        {
            ...
            cpr::Response response =
                cpr::Get(cpr::Url("https://raw.githubusercontent.com/"
                         ...
                         cpr::Verbose(false));
            std::cout << "Download completed, status code: "
            ...
            if (response.status_code == 200)
            {
                if (response.text != "")
                {
                    ...
                    try
                    {
                        ...
                        {
                            cout
                                << "Application is up-to-date! Remote version: "
                                << version << endl;
                        }
                    }
                    ...
                    {
                        cout << "Version number out of range: " << e.what()
                             << endl;
                    }
                }
                else
    ...
    if (appConfig.DefaultStyleFilepath != "")
        MyUtils::LoadImGuiColorsFromFilepath(style,
    ...
    if (useDarkMode)
    {
        if (FAILED(DwmSetWindowAttribute(hwnd,
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (IsMinimized && appConfig.HideToTray ||
            IsMinimized && !appConfig.HideToTray &&
                appConfig.HideWindowOnStartup && WindowHiddenOnStartup)
        {
        ...
        {
            if (ImGui::Begin(strings.StyleEditor.c_str(),
                             &ShowStyleEditWindow,
                             ImGuiWindowFlags_NoCollapse))
            {
                ...
                for (int i = 0; i < ImGuiCol_COUNT; ++i)
                {
                    ...
                    ImGui::SliderFloat(
                        std::string(std::string("R##") + colorName).c_str(),
                        &color->x,
                        0.0f,
                        1.0f);
                    ImGui::SameLine();
                    ImGui::SliderFloat(
                        std::string(std::string("G##") + colorName).c_str(),
                        &color->y,
                        0.0f,
                        1.0f);
                    ImGui::SameLine();
                    ImGui::SliderFloat(
                        std::string(std::string("B##") + colorName).c_str(),
                        &color->z,
                        0.0f,
                        1.0f);
                    ImGui::SameLine();
                    ImGui::SliderFloat(
                        std::string(std::string("A##") + colorName).c_str(),
                        &color->w,
                        0.0f,
                        1.0f);
                    ImGui::PopItemWidth();
        ...
        if (ImGui::Begin("Main", nullptr, window_flags))
        {
            ...
            if (ImGui::BeginCombo("##", CurrentController.c_str()))
            {
                for (int n = 0; n < ControllerID.size(); n++)
                {
                    if (ControllerID[n] != "")
                    {
                        ...
                        if (ImGui::Selectable(ControllerID[n].c_str(),
                                              is_selected))
                        {
            ...
            else
            {
                ImGui::SameLine();
                ImGui::Text(
                    std::string(strings.UDPStatus + ": " + strings.Active)
                        .c_str());
                ImGui::SameLine();
                ...
                if (!IsPresent)
                {
                    ...
                    if (x.Connected)
                    {
                        ...
                        if (configPath != "")
                        {
                            ...
                            if (s.emuStatus != None)
                            {
                                MyUtils::RunAsyncHidHideRequest(
                                    DualSense.back().GetPath(),
                                    "hide");
                            }
                        }
            ...
            for (int i = 0; i < ActiveDualsenseCount; i++)
            {
                ...
                if (DualSense[i].GetPath() == CurrentController &&
                        ...
                        if (s.ControllerInput.ID == CurrentController)
                        {
                            if (ImGui::BeginMainMenuBar())
                            {
                                        ...
                                        strings.ConfigHeader.c_str()))
                                {
                                    ...
                                    if (ImGui::Button(
                                            strings.SetDefaultConfig.c_str()))
                                    {
                                        std::string configPath =
                                            Config::GetConfigPath();
                                        if (configPath != "")
                                        {
                                            MyUtils::WriteDefaultConfigPath(
                                                configPath,
                                                DualSense[i].GetPath());
                                        }
                                    }
                                    Tooltip(strings.Tooltip_SetDefaultConfig
                                                .c_str());

                                    if (ImGui::Button(
                                            strings.RemoveDefaultConfig
                                                .c_str()))
                                    {
                                        MyUtils::RemoveConfig(
                                            DualSense[i].GetPath());
                                    }
                                    Tooltip(strings.Tooltip_RemoveDefaultConfig
                                                .c_str());

                                    if (ImGui::Checkbox(
                                            strings.RunAsAdmin.c_str(),
                                    ...
                                    if (ImGui::Checkbox(
                                            strings.HideWindowOnStartup.c_str(),
                                            &appConfig.HideWindowOnStartup))
                                    {
                                        Config::WriteAppConfigToFile(appConfig);
                                    }
                                    Tooltip(strings.Tooltip_HideWindowOnStartup
                                                .c_str());

                                    if (ImGui::Checkbox(
                                            strings.RunWithWindows.c_str(),
                                    ...
                                    if (ImGui::Checkbox(
                                            strings.DisconnectAllBTDevicesOnExit
                                                .c_str(),
                                            &appConfig
                                                 .DisconnectAllBTControllersOnExit))
                                    {
                                        Config::WriteAppConfigToFile(appConfig);
                                    }
                                ...
                                {
                                    if (ImGui::Button(
                                            strings.SaveStyleToFile.c_str()))
                                    {
                                        MyUtils::SaveImGuiColorsToFile(style);
                                    }

                                    if (ImGui::Button(
                                            strings.LoadStyleFromFile.c_str()))
                                    {
                                        MyUtils::LoadImGuiColorsFromFile(style);
                                    }

                                    if (ImGui::Button(
                                            strings.SetDefaultStyleFile
                                                .c_str()))
                                    {
                                        ...
                                        if (stylePath != "")
                                        {
                                            appConfig.DefaultStyleFilepath =
                                                stylePath;
                                            Config::WriteAppConfigToFile(
                                                appConfig);
                                        }
                                    }

                                    if (ImGui::Button(
                                            strings.RemoveDefaultStyle.c_str()))
                                    {
                                        appConfig.DefaultStyleFilepath = "";
                                        Config::WriteAppConfigToFile(appConfig);
                                    }
                                    ...
                                    if (ImGui::Button(
                                            strings.ResetStyle.c_str()))
                                    {
                                        style = ImGuiStyle();
                                    }

                                    if (ImGui::Button(
                                            strings.ShowEditMenu.c_str()))
                                    {
                            ...
                            DualSense[i].SetSoundVolume(
                                "screenshot",
                                s.ScreenshotSoundLoudness);
                            DualsenseUtils::HapticFeedbackStatus HF_status =
                                DualSense[i].GetHapticFeedbackStatus();
                            const char *bt_or_usb =
                                DualSense[i].GetConnectionType() == Feature::USB
                                    ? "USB"
                                    : "BT";
                            ImGui::Text("%s %d | %s: %s | %s: %d%%",
                                        strings.ControllerNumberText.c_str(),
                                        i + 1,
                                        strings.USBorBTconnectionType.c_str(),
                                        bt_or_usb,
                                        strings.BatteryLevel.c_str(),
                                        DualSense[i].State.battery.Level);

                            if (HF_status ==
                                DualsenseUtils::HapticFeedbackStatus::
                                    BluetoothNotUSB)
                            {
                                if (ImGui::Button(strings.DisconnectBT.c_str()))
                                {
                                    MyUtils::DisableBluetoothDevice(
                                        DualSense[i].GetMACAddress(false));
                                }
                            }
                            ...
                            if (UpdateAvailable)
                            {
                                ImGui::SameLine();
                                if (ImGui::Button(
                                        strings.InstallLatestUpdate.c_str()))
                                {
                                    std::cout << "Downloading updater.exe"
                                              << std::endl;
                                    cpr::Response response = cpr::Get(cpr::Url{
                                        "https://github.com/WujekFoliarz/"
                                        "DualSenseY-v2/raw/refs/heads/master/"
                                        "Updater.exe"});
                                    if (response.status_code == 200)
                                    {
                                        std::ofstream outFile("Updater.exe",
                                                              std::ios::binary);
                                        if (outFile)
                                        {
                                            outFile
                                                << response
                                                       .text; // Write the response content to the file
                                            outFile.close();
                                            std::cout
                                                << "File downloaded "
                                                   "successfully: Updater.exe"
                                                << std::endl;
                                        }
                                        else
                                        {
                            ...
                            if (!udpServer.isActive || !s.UseUDP)
                            {
                                     ...
                                     s.OverrideDS4Lightbar))
                                {
                                    if (ImGui::CollapsingHeader(
                                            strings.LedSection.c_str()))
                                    {
                                        ImGui::Separator();

                                        ImGui::Checkbox(
                                            strings.DisablePlayerLED.c_str(),
                                            &s.DisablePlayerLED);
                                        ...
                                        if (ImGui::BeginCombo(
                                                strings.PlayerLedBrightness.c_str(),
                                                playerLedBrightnessPreview))
                                        {
                                            if (ImGui::Selectable("+++"))
                                            {
                                        ...
                                        {
                                            ImGui::Checkbox(
                                                strings.AudioToLED.c_str(),
                                                &s.AudioToLED);
                                            Tooltip(strings.Tooltip_AudioToLED
                                                        .c_str());
                                        }
                                        ...
                                        {
                                            ImGui::Checkbox(
                                                strings.DiscoMode.c_str(),
                                                &s.DiscoMode);
                                            Tooltip(strings.Tooltip_DiscoMode
                                                        .c_str());
                                            ImGui::SameLine();
                                            ImGui::SetNextItemWidth(100);
                                            ImGui::SliderInt(
                                                std::string(strings.Speed +
                                                            "##Disco")
                                                    .c_str(),
                                                &s.DiscoSpeed,
                                                1,
                                        ...
                                        if (s.AudioToLED)
                                        {
                                                        ...
                                                        Working)
                                            {
                                                if (!s.SpeakerToLED)
                                                {
                                                    ImGui::Checkbox(
                                                        strings
                                                            .UseControllerActuatorsInsteadOfSystemAudio
                                                            .c_str(),
                                                        &s.HapticsToLED);
                                                }
                                                if (!s.HapticsToLED)
                                                {
                                                    ImGui::Checkbox(
                                                        strings
                                                            .UseControllerSpeakerInsteadOfSystemAudio
                                                            .c_str(),
                                                        &s.SpeakerToLED);
                                                }
                                                if (s.SpeakerToLED ||
                                            ...
                                            ImGui::Text(
                                                strings.QuietColor.c_str());
                                            ImGui::SliderInt(
                                                string(strings.LED_Red + "##q")
                                                    .c_str(),
                                                &s.QUIET_COLOR[0],
                                                0,
                                                255);
                                            ImGui::SliderInt(
                                                string(strings.LED_Green +
                                                       "##q")
                                                    .c_str(),
                                                &s.QUIET_COLOR[1],
                                                0,
                                            ...
                                            ImGui::Text(
                                                strings.MediumColor.c_str());
                                            ImGui::SliderInt(
                                                string(strings.LED_Red + "##m")
                                                    .c_str(),
                                                &s.MEDIUM_COLOR[0],
                                                0,
                                                255);
                                            ...
                                            ImGui::Text(
                                                strings.LoudColor.c_str());
                                            ImGui::SliderInt(
                                                string(strings.LED_Red + "##l")
                                                    .c_str(),
                                                &s.LOUD_COLOR[0],
                                                0,
                                                255);
                                        ...
                                        {
                                            ImGui::Checkbox(
                                                strings.BatteryLightbar.c_str(),
                                                &s.BatteryLightbar);
                                            Tooltip(
                                                strings.Tooltip_BatteryLightbar
                                                    .c_str());
                                        }
                                ...
                                {
                                    ImGui::Text(
                                        strings.LED_DS4_Unavailable.c_str());
                                }

                                if (ImGui::CollapsingHeader(
                                        strings.AdaptiveTriggers.c_str()))
                                {
                                    ImGui::Checkbox(strings.RumbleToAT.c_str(),
                                                    &s.RumbleToAT);
                                    ...
                                    {
                                        ImGui::Checkbox(
                                            strings.HapticsToTriggers.c_str(),
                                            &s.HapticsToTriggers);
                                        Tooltip(
                                            strings.Tooltip_HapticsToTriggers
                                                .c_str());
                                        if (s.HapticsToTriggers)
                                        {
                                            ...
                                            ImGui::SliderFloat(
                                                strings.Scale.c_str(),
                                                &s.AudioToTriggersMaxFloatValue,
                                                0.01f,
                                                10.0f);
                                        }
                                    }
                                    ...
                                    {
                                        ImGui::Checkbox(
                                            strings.FullyRetractWhenNoData
                                                .c_str(),
                                            &s.FullyRetractTriggers);

                                        ImGui::Checkbox(
                                            strings.SwapTriggersRumbleToAT
                                                .c_str(),
                                            &s.SwapTriggersRumbleToAT);
                                        Tooltip(
                                            strings
                                                .Tooltip_SwapTriggersRumbleToAT
                                                .c_str());

                                        ImGui::Checkbox(
                                            strings.RumbleToAT_RigidMode
                                                .c_str(),
                                            &s.RumbleToAT_RigidMode);
                                        Tooltip(
                                            strings.Tooltip_RumbleToAT_RigidMode
                                                .c_str());

                                        if (!s.RumbleToAT_RigidMode)
                                        {
                                            ImGui::SliderInt(
                                                strings.MaxLeftIntensity
                                                    .c_str(),
                                                &s.MaxLeftRumbleToATIntensity,
                                                0,
                                                255);
                                            Tooltip(strings.Tooltip_MaxIntensity
                                                        .c_str());

                                            ImGui::SliderInt(
                                                strings.MaxLeftFrequency
                                                    .c_str(),
                                                &s.MaxLeftRumbleToATFrequency,
                                                0,
                                                25);
                                            Tooltip(strings.Tooltip_MaxFrequency
                                                        .c_str());

                                            ImGui::Separator();

                                            ImGui::SliderInt(
                                                strings.MaxRightIntensity
                                                    .c_str(),
                                                &s.MaxRightRumbleToATIntensity,
                                                0,
                                                255);
                                            Tooltip(strings.Tooltip_MaxIntensity
                                                        .c_str());
                                            ...
                                            ImGui::SliderInt(
                                                strings.MaxRightFrequency
                                                    .c_str(),
                                                &s.MaxRightRumbleToATFrequency,
                                                0,
                                                25);
                                            Tooltip(strings.Tooltip_MaxFrequency
                                                        .c_str());
                                    ...
                                    if (!s.RumbleToAT && !s.HapticsToTriggers)
                                    {
                                        ...
                                        if (ImGui::BeginCombo(
                                                strings.TriggerFormat.c_str(),
                                                s.triggerFormat.c_str()))
                                        {
                                            if (ImGui::Selectable("Sony",
                                                                  false))
                                            {
                                            ...
                                            if (s.curTrigger == "L2")
                                            {
                                                         ...
                                                         "Weapon")
                                                {
                                                    ImGui::SetNextItemWidth(
                                                        300);
                                                    ImGui::SliderInt(
                                                        strings.StartPosition
                                                            .c_str(),
                                                        &s.L2WeaponFeedback[0],
                                                        2,
                                                        7);

                                                    if (s.L2WeaponFeedback[1] <
                                                        s.L2WeaponFeedback[0])
                                            ...
                                            else if (s.curTrigger == "R2")
                                            {
                                                         ...
                                                         "Weapon")
                                                {
                                                    ...
                                                    ImGui::SliderInt(
                                                        strings.StartPosition
                                                            .c_str(),
                                                        &s.R2WeaponFeedback[0],
                                                        2,
                                                        7);

                                                    if (s.R2WeaponFeedback[1] <
                                                        s.R2WeaponFeedback[0])
                            ...
                            {
                                ImGui::Text(
                                    strings.LEDandATunavailableUDP.c_str());
                            }
                            ...
                            if (ImGui::CollapsingHeader(strings.Motion.c_str()))
                            {
                                ...
                                ImGui::SliderInt(
                                    std::string(strings.Gyroscope + " X")
                                        .c_str(),
                                    &DualSense[i].State.gyro.X,
                                    -9000,
                                    9000);
                                ImGui::SetNextItemWidth(400);
                                ImGui::SliderInt(
                                    std::string(strings.Gyroscope + " Y")
                                        .c_str(),
                                    &DualSense[i].State.gyro.Y,
                                    -9000,
                                    9000);
                                ImGui::SetNextItemWidth(400);
                                ImGui::SliderInt(
                                    std::string(strings.Gyroscope + " Z")
                                        .c_str(),
                                    &DualSense[i].State.gyro.Z,
                                    -9000,
                                    9000);

                                ImGui::SetNextItemWidth(400);
                                ImGui::SliderInt(
                                    std::string(strings.Accelerometer + " X")
                                        .c_str(),
                                    &DualSense[i].State.accelerometer.X,
                                    -9000,
                                    9000);
                                ImGui::SetNextItemWidth(400);
                                ImGui::SliderInt(
                                    std::string(strings.Accelerometer + " Y")
                                        .c_str(),
                                    &DualSense[i].State.accelerometer.Y,
                                    -9000,
                                    9000);
                                ImGui::SetNextItemWidth(400);
                                ImGui::SliderInt(
                                    std::string(strings.Accelerometer + " Z")
                                        .c_str(),
                                    &DualSense[i].State.accelerometer.Z,
                                    -9000,
                                    9000);
                                ImGui::EndDisabled();

                                ImGui::Separator();
                                ImGui::Checkbox(strings.GyroToMouse.c_str(),
                                                &s.GyroToMouse);
                                ImGui::SameLine();
                                Tooltip(strings.Tooltip_GyroToMouse.c_str());
                                ImGui::SetNextItemWidth(250);
                                ImGui::SliderFloat(
                                    std::string(strings.Sensitivity +
                                                "##gyrotomouse")
                                        .c_str(),
                                    &s.GyroToMouseSensitivity,
                                    0.001f,
                                    0.030f);
                                ...
                                if (s.emuStatus == None)
                                    ImGui::Text(
                                        strings.ControllerEmulationRequired
                                            .c_str());
                                ImGui::Checkbox(
                                    strings.GyroToRightAnalogStick.c_str(),
                                    &s.GyroToRightAnalogStick);
                                Tooltip(strings.Tooltip_GyroToRightAnalogStick
                                            .c_str());
                                ImGui::SetNextItemWidth(250);
                                ImGui::SliderFloat(
                                    std::string(strings.Sensitivity +
                                        ...
                                        .c_str(),
                                    &s.GyroToRightAnalogStickSensitivity,
                                    0.1f,
                                    5.0f);
                                ImGui::SetNextItemWidth(250);
                                ImGui::SliderFloat(
                                    strings.GyroDeadzone.c_str(),
                                    &s.GyroToRightAnalogStickDeadzone,
                                    0.001,
                                    0.300);
                                ImGui::SetNextItemWidth(250);
                                ImGui::SliderInt(
                                    strings.MinimumStickValue.c_str(),
                                    &s.GyroToRightAnalogStickMinimumStickValue,
                                    1,
                                    127);
                                    ...
                                    strings.HapticFeedback.c_str()))
                            {
                                ...
                                ImGui::SetNextItemWidth(300);
                                ImGui::SliderInt(
                                    strings.RumbleReduction.c_str(),
                                    &s.ControllerInput.RumbleReduction,
                                    0x0,
                                    0x7);

                                ImGui::Checkbox(
                                    strings.ImprovedRumbleEmulation.c_str(),
                                    &s.ControllerInput.ImprovedRumbleEmulation);
                              
                                ImGui::Separator();

                                if (ImGui::Checkbox(
                                        strings.RunAudioToHapticsOnStart
                                            .c_str(),
                                        &s.RunAudioToHapticsOnStart))
                                {
                                ...
                                {
                                    ImGui::Checkbox(
                                        strings.TouchpadToHaptics.c_str(),
                                        &s.TouchpadToHaptics);
                                    DualSense[i].TouchpadToHaptics =
                                        s.TouchpadToHaptics;
                                    ImGui::SameLine();
                                    ImGui::SetNextItemWidth(100);
                                    ImGui::SliderFloat(
                                        strings.Frequency.c_str(),
                                        &s.TouchpadToHapticsFrequency,
                                        1.0f,
                                        440.0f);
                                    DualSense[i].TouchpadToHapticsFrequency =
                                        s.TouchpadToHapticsFrequency;

                                    if (ImGui::Button(
                                            strings.StartAudioToHaptics
                                                .c_str()))
                                    {
                                        MyUtils::StartAudioToHaptics(
                                            DualSense[i].GetAudioDeviceName());
                                    };
                                    Tooltip(strings.Tooltip_StartAudioToHaptics
                                                .c_str());

                                    ImGui::SetNextItemWidth(300);
                                    ImGui::SliderInt(
                                        strings.SpeakerVolume.c_str(),
                                        &s.ControllerInput.SpeakerVolume,
                                        0,
                                        0x7C,
                                        "%d");
                                    Tooltip(
                                        strings.Tooltip_SpeakerVolume.c_str());
                                    ImGui::SetNextItemWidth(300);
                                    ImGui::SliderInt(
                                        strings.HeadsetVolume.c_str(),
                                        &s.ControllerInput.HeadsetVolume,
                                        0,
                                        0x7C,
                                        "%d");

                                    ImGui::Checkbox(strings.UseHeadset.c_str(),
                                                    &s.UseHeadset);

                                    ImGui::Separator();
                                    ImGui::Text(
                                        MyUtils::WStringToString(
                                            DualSense[i]
                                                .GetAudioDeviceInstanceID())
                                            .c_str());
                                    ImGui::Text(
                                        DualSense[i].GetAudioDeviceName());
                                }
                                else
                                {
                                    ...
                                    {
                                        ImGui::Text(
                                            strings.HapticsUnavailable.c_str());
                                    }
                                    else if (HF_status ==
                                             DualsenseUtils::
                                                 HapticFeedbackStatus::
                                                     AudioDeviceNotFound)
                                    {
                                        ImGui::Text(
                                            strings
                                                .HapticsUnavailableNoAudioDevice
                                                .c_str());
                                    }
                                    else if (HF_status ==
                                             DualsenseUtils::
                                                     ...
                                                     AudioEngineNotInitialized)
                                    {
                                        ImGui::Text(
                                            strings
                                                .AudioEngineNotInitializedError
                                                .c_str());
                                    }
                                }
                            }
                            ...
                            if (ImGui::CollapsingHeader(
                                    strings.AnalogSticks.c_str()))
                            {
                                ImVec2 cursor_pos = ImGui::GetCursorScreenPos();
                                float radius = 200.0f;
                                ImVec2 center = ImVec2(radius + cursor_pos.x,
                                                       radius + cursor_pos.y);
                                ...
                                ImGui::GetWindowDrawList()->AddCircle(
                                    center,
                                    radius,
                                    DualSense[i].State.L3 ? colorRed : color);
                                ImGui::GetWindowDrawList()->AddCircle(
                                    center,
                                    ConvertRange(s.LeftAnalogDeadzone,
                                                 0,
                                                 127,
                                                 0,
                                                 radius),
                                ...
                                ImVec2 leftStickPos = ImVec2(
                                    center.x + ConvertRange(
                                                   DualSense[i].State.LX - 128,
                                                   -128,
                                                   128,
                                                   -radius,
                                                   radius),
                                    center.y + ConvertRange(
                                                   DualSense[i].State.LY - 128,
                                                   -128,
                                                   128,
                                                   -radius,
                                                   radius));
                                ...
                                sprintf(left_text,
                                        "X: %d | Y: %d",
                                        DualSense[i].State.LX,
                                        DualSense[i].State.LY);
                                ImVec2 left_text_size =
                                    ImGui::CalcTextSize(left_text);
                                ImVec2 left_text_pos =
                                    ImVec2(center.x - left_text_size.x / 2,
                                           center.y + radius);
                                ...
                                ImVec2 right_center =
                                    ImVec2(radius + cursor_pos.x,
                                           radius + cursor_pos.y);

                                ImGui::GetWindowDrawList()->AddCircle(
                                    right_center,
                                    radius,
                                    DualSense[i].State.R3 ? colorRed : color);
                                ImGui::GetWindowDrawList()->AddCircle(
                                    right_center,
                                    ConvertRange(s.RightAnalogDeadzone,
                                                 0,
                                                 127,
                                                 0,
                                                 radius),
                                ...
                                ImVec2 rightStickPos =
                                    ImVec2(right_center.x +
                                               ConvertRange(
                                                   DualSense[i].State.RX - 128,
                                                   -128,
                                                   128,
                                                   -radius,
                                                   radius),
                                ...
                                sprintf(right_text,
                                        "X: %d | Y: %d",
                                        DualSense[i].State.RX,
                                        DualSense[i].State.RY);
                                ImVec2 right_text_size =
                                    ImGui::CalcTextSize(right_text);
                                ImVec2 right_text_pos = ImVec2(
                                    right_center.x - right_text_size.x / 2,
                                ...
                                ImGui::Dummy(
                                    ImVec2(radius * 2, (radius * 2) + 40));
                                ImGui::Separator();

                                ImGui::Text(strings.GeneralSettings.c_str());
                                if (!s.RightStickToMouse)
                                    ImGui::Checkbox(
                                        std::string(strings.LeftStickToMouse +
                                                    "##L3toMouse")
                                            .c_str(),
                                        &s.LeftStickToMouse);
                                if (!s.LeftStickToMouse)
                                    ImGui::Checkbox(
                                        std::string(strings.RightStickToMouse +
                                                    "##R3toMouse")
                                            .c_str(),
                                        &s.RightStickToMouse);
                                ImGui::SetNextItemWidth(300);
                                ImGui::SliderFloat(
                                    std::string(strings.Sensitivity +
                                                "##AnalogStickToMouse")
                                        .c_str(),
                                    &s.LeftStickToMouseSensitivity,
                                    0.01f,
                                    0.20f);

                                ImGui::Text(
                                    strings.EmulatedControllerSettings.c_str());
                                ImGui::SetNextItemWidth(300);
                                ImGui::SliderInt(
                                    strings.LeftAnalogStickDeadZone.c_str(),
                                    &s.LeftAnalogDeadzone,
                                    0,
                                    127);
                                ImGui::SetNextItemWidth(300);
                                ImGui::SliderInt(
                                    strings.RightAnalogStickDeadZone.c_str(),
                                    &s.RightAnalogDeadzone,
                                    0,
                                    127);
                                ImGui::Checkbox(
                                    strings.TouchpadAsRightStick.c_str(),
                                    &s.TouchpadToRXRY);
                            }

                            if (ImGui::CollapsingHeader(
                                    strings._Touchpad.c_str()))
                            {
                                // Define the touchpad's preview size in the UI
                                ImVec2 touchpadSize(
                                    300 * io.FontGlobalScale,
                                    140 *
                                        io.FontGlobalScale); // Adjusted to keep a similar aspect ratio (1920:900)

                                // Draw touchpad background
                                ImGui::InvisibleButton("##touchpad_bg",
                                                       touchpadSize);
                                ImDrawList *drawList =
                                    ImGui::GetWindowDrawList();
                                ImVec2 touchpadPos = ImGui::GetItemRectMin();
                                int rcolor =
                                    DualSense[i].State.touchBtn ? 100 : 50;
                                ...
                                ImGui::Checkbox(strings.TouchpadToMouse.c_str(),
                                                &s.TouchpadToMouse);
                                Tooltip(
                                    strings.Tooltip_TouchpadToMouse.c_str());
                                ImGui::SetNextItemWidth(300);
                                    ...
                                    strings.MicButton.c_str()))
                            {
                                        ...
                                        Working)
                                {
                                    ...
                                    ImGui::SetNextItemWidth(100);
                                    ImGui::SliderFloat(
                                        strings.ScreenshotSoundVolume.c_str(),
                                        &s.ScreenshotSoundLoudness,
                                        0,
                                        1);
                                }
                                ...
                                ImGui::Checkbox(
                                    strings.RealMicFunctionality.c_str(),
                                    &s.MicFunc);
                                Tooltip(strings.Tooltip_RealMicFunctionality
                                            .c_str());
                                ...
                                ImGui::Checkbox(
                                    strings.TouchpadShortcut.c_str(),
                                    &s.TouchpadToMouseShortcut);
                                ImGui::Checkbox(
                                    strings.SwapTriggersShortcut.c_str(),
                                    &s.SwapTriggersShortcut);
                                ImGui::Checkbox(strings.X360Shortcut.c_str(),
                                                &s.X360Shortcut);
                                ImGui::Checkbox(strings.DS4Shortcut.c_str(),
                                                &s.DS4Shortcut);
                                ImGui::Checkbox(strings.StopEmuShortcut.c_str(),
                                                &s.StopEmuShortcut);
                                ImGui::Checkbox(
                                    strings.StopWritingShortcut.c_str(),
                                    &s.StopWritingShortcut);
                                ImGui::Checkbox(
                                    strings.GyroToMouseShortcut.c_str(),
                                    &s.GyroToMouseShortcut);
                                ImGui::Checkbox(
                                    strings.GyroToRightAnalogStickShortcut
                                        .c_str(),
                                    &s.GyroToRightAnalogStickShortcut);
                                ImGui::Checkbox(
                                    strings.DisconnectControllerShortcut
                                        .c_str(),
                                    &s.DisconnectControllerShortcut);
                            }

                            if (ImGui::CollapsingHeader(
                                    strings.EmulationHeader.c_str()))
                            {
                                if (s.emuStatus == None)
                                {
                                    ...
                                    ImGui::SameLine();
                                    ImGui::Checkbox(
                                        strings.DualShock4V2emu.c_str(),
                                        &s.DualShock4V2);
                                    Tooltip(
                                        strings.Tooltip_Dualshock4V2.c_str());
                                }
                                else
                                {
                                ...
                                {
                                    ImGui::Text(
                                        strings.ControllerEmuUserMode.c_str());
                                }
                                else
                                {
                                    ImGui::Text(strings.ControllerEmuAppAsAdmin
                                                    .c_str());
                                }
                                ...
                                ImGui::SetNextItemWidth(300);
                                ImGui::SliderInt(strings.L2Deadzone.c_str(),
                                                 &s.L2Deadzone,
                                                 1,
                                                 255);
                                ImGui::SetNextItemWidth(300);
                                ImGui::SliderInt(strings.R2Deadzone.c_str(),
                                                 &s.R2Deadzone,
                                                 1,
                                                 255);
                                ImGui::Checkbox(
                                    strings.TriggersAsButtons.c_str(),
                                    &s.TriggersAsButtons);
                                Tooltip(
                                    strings.Tooltip_TriggersAsButtons.c_str());
                                ImGui::Checkbox(
                                    strings.TouchpadAsSelect.c_str(),
                                    &s.TouchpadAsSelect);
                                ImGui::Checkbox(strings.TouchpadAsStart.c_str(),
                                                &s.TouchpadAsStart);
                                ImGui::Checkbox(
                                    strings.OverrideDS4Lightbar.c_str(),
                                    &s.OverrideDS4Lightbar);
                            }
