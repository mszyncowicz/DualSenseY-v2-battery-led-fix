...
#pragma endregion
		...
		skiphapticstotriggers:
			...
			skiprumbleretract:
				if (settings.HapticsToTriggers) {
					if (settings.lrumble > 0 || settings.rrumble > 0) goto skipretract;
				...
				if (DualSense[i].GetPath() == CurrentController &&
					...
					for (Settings& s : ControllerSettings) {
						if (s.ControllerInput.ID == CurrentController) {
								...
								if (ImGui::BeginMenu(strings.ConfigHeader.c_str())) {
									...
									if (ImGui::Button(strings.LoadConfig.c_str())) {
										...
										if (configPath != "") {
											s = Config::ReadFromFile(configPath);
											if (s.emuStatus == None) {
												MyUtils::RunAsyncHidHideRequest(DualSense[i].GetPath(),
																				"show");
											}
											else {
												MyUtils::RunAsyncHidHideRequest(DualSense[i].GetPath(),
							...
							DualSense[i].SetSoundVolume("screenshot",
							...
							const char* bt_or_usb =
								DualSense[i].GetConnectionType() == Feature::USB ? "USB"
								: "BT";
							ImGui::Text("%s %d | %s: %s | %s: %d%%",
										strings.ControllerNumberText.c_str(), i + 1,
							...
							if (UpdateAvailable) {
								...
								if (ImGui::Button(strings.InstallLatestUpdate.c_str())) {
									...
									cpr::Response response =
										cpr::Get(cpr::Url{ "https://github.com/WujekFoliarz/"
														  "DualSenseY-v2/raw/refs/heads/master/"
														  "Updater.exe" });
									if (response.status_code == 200) {
										std::ofstream outFile("Updater.exe", std::ios::binary);
										if (outFile) {
											outFile << response.text;  // Write the response content
											// to the file
											outFile.close();
							...
							if (ImGui::CollapsingHeader(strings.Motion.c_str())) {
								...
								ImGui::SliderInt(std::string(strings.Gyroscope + " X").c_str(),
												 &DualSense[i].State.gyro.X, -9000, 9000);
								ImGui::SetNextItemWidth(400);
								ImGui::SliderInt(std::string(strings.Gyroscope + " Y").c_str(),
												 &DualSense[i].State.gyro.Y, -9000, 9000);
								ImGui::SetNextItemWidth(400);
								ImGui::SliderInt(std::string(strings.Gyroscope + " Z").c_str(),
												 &DualSense[i].State.gyro.Z, -9000, 9000);

								ImGui::SetNextItemWidth(400);
								ImGui::SliderInt(
									std::string(strings.Accelerometer + " X").c_str(),
									&DualSense[i].State.accelerometer.X, -9000, 9000);
								ImGui::SetNextItemWidth(400);
								ImGui::SliderInt(
									std::string(strings.Accelerometer + " Y").c_str(),
									&DualSense[i].State.accelerometer.Y, -9000, 9000);
								ImGui::SetNextItemWidth(400);
								ImGui::SliderInt(
									std::string(strings.Accelerometer + " Z").c_str(),
									&DualSense[i].State.accelerometer.Z, -9000, 9000);
								ImGui::EndDisabled();
							...
							if (ImGui::CollapsingHeader(strings.HapticFeedback.c_str())) {
								...
								if (HF_status ==
									...
									ImGui::Text(MyUtils::WStringToString(
										DualSense[i].GetAudioDeviceInstanceID())
													.c_str());
							...
							if (ImGui::CollapsingHeader(strings.AnalogSticks.c_str())) {
								...
								ImGui::GetWindowDrawList()->AddCircle(
									center, radius, DualSense[i].State.L3 ? colorRed : color);
								ImGui::GetWindowDrawList()->AddCircle(
									center,
								...
								ImVec2 leftStickPos =
									ImVec2(center.x + ConvertRange(DualSense[i].State.LX - 128,
										   -128, 128, -radius, radius),
										   center.y + ConvertRange(DualSense[i].State.LY - 128,
										   -128, 128, -radius, radius));
								ImGui::GetWindowDrawList()->AddCircleFilled(leftStickPos, 10.0f,
																			colorRed);

								char left_text[32];
								sprintf(left_text, "X: %d | Y: %d", DualSense[i].State.LX,
								...
								ImVec2 rightStickPos = ImVec2(
									right_center.x + ConvertRange(DualSense[i].State.RX - 128,
									-128, 128, -radius, radius),
									right_center.y + ConvertRange(DualSense[i].State.RY - 128,
									-128, 128, -radius, radius));
								ImGui::GetWindowDrawList()->AddCircleFilled(rightStickPos,
																			10.0f, colorRed);

								char right_text[32];
								sprintf(right_text, "X: %d | Y: %d", DualSense[i].State.RX,
...
// Draw touchpad background
								...
								if (DualSense[i].State.trackPadTouch0.IsActive) {
									float scaledX =
										(DualSense[i].State.trackPadTouch0.X / 1900.0f) *
										touchpadSize.x;
									float scaledY =
										(DualSense[i].State.trackPadTouch0.Y / 1100.0f) *
										touchpadSize.y;
								...
								if (DualSense[i].State.trackPadTouch1.IsActive) {
									float scaledX =
										(DualSense[i].State.trackPadTouch1.X / 1900.0f) *
										touchpadSize.x;
									float scaledY =
										(DualSense[i].State.trackPadTouch1.Y / 1100.0f) *
										touchpadSize.y;
									ImVec2 touchPos(touchpadPos.x + scaledX,
													touchpadPos.y + scaledY);
								...
								ImGui::Text(
									...
									DualSense[i].State.TouchPacketNum, strings.Touch.c_str(),
									DualSense[i].State.trackPadTouch0.X,
									DualSense[i].State.trackPadTouch0.Y, strings.Touch.c_str(),
	...
	for (Dualsense& ds : DualSense) {
		if (WasElevated) MyUtils::StartHidHideRequest(ds.GetPath(), "show");
